...
//        server.stop();
        ...
        if(powerStr == "off"){
            server.send(200, "application/json; charset=utf-8", "{\"status\":\"off\"}");
            for(int i = 1; i<=255; i++){
            ...
            server.send(200, "application/json; charset=utf-8", "{\"status\":\"on\"}");
            for(int i = 1; i<=255; i++){
            ...
            server.send(200, "application/json; charset=utf-8", "{\"status\":\"change\"}");
            ...
            server.send(200, "application/json; charset=utf-8", "{\"status\":\"cant\"}");
...
//            server.begin();
        ...
        ECHO("Wrote: ");
        EEPROM.write(EEPROM_WIFI_LED_RED, uint8_t(redStr.toInt()));
        
        ECHOLN("");
        ECHOLN("writing eeprom LED GREEN:");
        ECHO("Wrote: ");
        EEPROM.write(EEPROM_WIFI_LED_GREEN, uint8_t(greenStr.toInt()));
        
        ECHOLN("");
        ECHOLN("writing eeprom LED BLUE:"); 
        ECHO("Wrote: ");
        EEPROM.write(EEPROM_WIFI_LED_BLUE, uint8_t(blueStr.toInt()));
...
void ControlLedColor(){
    ...
    if(flag_led == false){
        server.send(200, "application/json; charset=utf-8", "{\"status\":\"led is off\"}");
        return;
    }
    if (rootData.success()) {
        // server.sendHeader("Access-Control-Allow-Headers", "*");
        ...
        ledcWrite(LED_CHANNEL_R, uint8_t(red_after));
        ledcWrite(LED_CHANNEL_G, uint8_t(green_after));
        ledcWrite(LED_CHANNEL_B, uint8_t(blue_after));
...
//        server.stop();
        String intensityLightStr = rootData["intensityLight"];
        intensityLight = atoi(intensityLightStr.c_str());
        ...
        // ECHOLN(out_led_red);
        red_after = uint8_t(out_led_red);
        ledcWrite(LED_CHANNEL_R, red_after);

        out_led_green = ((float)green_after*intensityLight)/100;
        green_after = uint8_t(out_led_green);
        ledcWrite(LED_CHANNEL_G, green_after);

        out_led_blue = ((float)blue_after*(float)intensityLight)/100;
        blue_after = uint8_t(out_led_blue);
        ledcWrite(LED_CHANNEL_B, blue_after);
        // ECHO(red_after);
        // ECHO("---");
        // ECHO(green_after);
        // ECHO("---");
...
//        ECHOLN("LED IS ON!");
//        flag_led = true;
        ledcWrite(LED_CHANNEL_R, uint8_t((float)red_before*intensityLight/100));
        ledcWrite(LED_CHANNEL_G, uint8_t((float)green_before*intensityLight/100));
        ledcWrite(LED_CHANNEL_B, uint8_t((float)blue_before*intensityLight/100));
//    }
            ...
            if(rootData["typedevice"] == m_Typedevice){
                ...
                if(isTrueControl == true){
                    ...
                    if(dataType == "controlled"){
                        ...
                        ledcWrite(LED_CHANNEL_R, uint8_t((float)red_after*intensityLight/100));
                        ledcWrite(LED_CHANNEL_G, uint8_t((float)green_after*intensityLight/100));
                        ledcWrite(LED_CHANNEL_B, uint8_t((float)blue_after*intensityLight/100));
                        red_before = red_after;
                        green_before = green_after;
                        blue_before = blue_after;
                ...
                if(isTrueControl == true){
                    ...
                    //---------control------------------
                        ...
                        for(int i = 1; i<=255; i++){
                            ...
                            out_led_red = abs(out_led_red);
                            ledcWrite(LED_CHANNEL_R, uint8_t(out_led_red*intensityLight/100));
                            out_led_green = (float)0 + ((float)((float)green_after - (float)0)/255)*i;
                            out_led_green = abs(out_led_green);
                            ledcWrite(LED_CHANNEL_G, uint8_t(out_led_green*intensityLight/100));
                            out_led_blue = (float)0 + ((float)((float)blue_after - (float)0)/255)*i;
                            out_led_blue = abs(out_led_blue);
                            ledcWrite(LED_CHANNEL_B, uint8_t(out_led_blue*intensityLight/100));
                            delay(10);
                        }
                        ECHOLN("Wrote Led on");
                        EEPROM.write(EEPROM_WIFI_LED_STATUS_ON_OFF, 1);
...
void loop() {
    ...
    if(millis() > 10000 && millis() == timeToSaveData + 5000){
        delay(5);
        ECHOLN("Save data");
        ECHO("RED: ");
        ECHOLN(red_after);
