...
void motor_init(){
    ...
    if(control_motor.mode_run != 1 && control_motor.mode_run != 2 && control_motor.mode_run != 3 && control_motor.mode_run != 4 && control_motor.mode_run != 5 && control_motor.mode_run != 6){
        control_motor.mode_run = 4;
        ECHOLN("read set moderun EEPROM fail, auto set 4!");
    }else{
        ECHO("Mode Run =  ");
...
void setSpeedControl(){
    ...
    if(!control_motor.first_run && !control_motor.status_stop && control_motor.forward && control_motor.count_pul_FG < outside){
        ...
        case 4:
            ...
            break;
        case 5:
            ledcWrite(MOTOR_CHANNEL, MOTOR_MODE_SPEED_5);
            // ECHOLN("MOTOR_MODE_SPEED_5");
            break;
        case 6:
            ledcWrite(MOTOR_CHANNEL, MOTOR_MODE_SPEED_6);
            // ECHOLN("MOTOR_MODE_SPEED_6");
            break;
        default:
            break;
        }
