#include "cua_tu_esp_6_sensor_mqtt_tu_queen.h"


void getStatus(){
    String datasend = "{\"deviceid\" : \"";
    datasend += String(deviceId);
    datasend += "\", \"devicetypec\" : \"motor\", \"typecontrol\" : \"getstatus\",  \"number_disconnect\" : \"";
...
void loadDataBegin(){
    ...
    //distantOffLedQueen
    distantOffLedQueen = EEPROM.read(EEPROM_DISTANT_OFF_LED_QUEEN);
    if(distantOffLedQueen == 0 || distantOffLedQueen == 255){
        distantOffLedQueen = 10;       
    }
    ECHO("distantOffLedQueen: ");
    ECHOLN(distantOffLedQueen);
        ...
        if (rootData.success()){
            ...
            if(rootData["typedevice"] == m_Typedevice){
                ...
                if(isTrueControl == true){
                    ...
                    else if(dataType == "changedistantoffled"){
                        String StrdistantOffLedQueen = rootData["data"];
                        distantOffLedQueen = StrdistantOffLedQueen.toInt();      
                        ECHO("Writed: ");
                        ECHOLN(distantOffLedQueen);
                        EEPROM.write(EEPROM_DISTANT_OFF_LED_QUEEN, distantOffLedQueen);
                        EEPROM.commit();
                    }
                }
            }
...
void loop() {
    ...
    if(WiFi.status() == WL_CONNECTED){
        ...
        if(Forward == false && statusLed == true && statusStop == false  && countPulFGDistant <= (countPulDistant-distantOffLedQueen)){
            statusLed = false;

            String datasend = "{\"deviceid\" : [1], \"typedevice\" : \"ledRgb\", \"typecontrol\" : \"off\"}";
            String topic = "CabinetAvy/HPT/ledqueen";
            client.publish(topic.c_str(), datasend.c_str());
            

            tickerSetPwmLedLightOn.stop();
            tickerSetPwmLedLightOff.start();
        }
        //bat den led khi mo cua (cho tu queen)
        if(Forward == true && statusLed == false && statusStop == false  && countPulFGDistant >= 10){
            statusLed = true;

            String datasend = "{\"deviceid\" : [1], \"typedevice\" : \"ledRgb\", \"typecontrol\" : \"on\"}";
            String topic = "CabinetAvy/HPT/ledqueen";
            client.publish(topic.c_str(), datasend.c_str());
