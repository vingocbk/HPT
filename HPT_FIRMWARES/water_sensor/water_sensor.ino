...
void loop(void)
{
    if(isRegister){
        ...
        if(isCanSending == true && digitalRead(PIN_WATER) == LOW && isHaveWater == false){
            ...
            isHaveWater = true;
            isCanSending = false;
            digitalWrite(PIN_VAN, KHOA_VAN);
            digitalWrite(PIN_PUZZER, HIGH);
            delay(100);
            digitalWrite(PIN_PUZZER, LOW);
            delay(100);
            digitalWrite(PIN_PUZZER, HIGH);
            delay(100);
            digitalWrite(PIN_PUZZER, LOW);
